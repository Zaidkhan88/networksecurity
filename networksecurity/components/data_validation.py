...
class DataValidation:
    ...
    def numerical_columns_match(self,dataframe:pd.DataFrame)->bool:
        try:            
            numerixal_columns = self._schema_config["numerical_columns"]
            dataframe_numerical_columns = dataframe.select_dtypes(include=['int64','float64']).columns
            logging.info(f"Required numerical columns: {numerixal_columns}")
            logging.info(f"Dataframe has numerical columns: {list(dataframe_numerical_columns)}")
            for column in numerixal_columns:
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            status = self.numerical_columns_match(train_dataframe)
            if not status:
                raise Exception(f"Train dataframe does not have required numerical columns.")

            status = self.numerical_columns_match(test_dataframe)
            if not status:
